...
namespace BSP::Motor::DM
{
    ...
    /**
     * @brief J6006减速电机类 (DM-J6006-2EC)
     * 峰值扭矩: 12NM, 空载最大转速: 240rpm, 减速比: 6:1
     */
    template <uint8_t N> 
    ...
    public:
        J6006(uint16_t Init_id, const uint8_t (&ids)[N], const uint32_t (&send_idxs)[N])
            : DMMotorBase<N>(Init_id, ids, send_idxs, 
                            // 参数顺序: P_MIN, P_MAX, V_MIN, V_MAX, T_MIN, T_MAX, KP_MIN, KP_MAX, KD_MIN, KD_MAX
                            Parameters(-12.56f, 12.56f, -45.0f, 45.0f, -12.0f, 12.0f, 0.0f, 500.0f, 0.0f, 5.0f))
        {
        }
    };
